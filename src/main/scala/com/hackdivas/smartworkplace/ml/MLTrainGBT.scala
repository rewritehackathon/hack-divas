...
class MLTrainGBT(context: SmartWorkplaceContext) {
  ...
  def run = {
    ...
    val modelPath = config("ml.train.save.model.path.GBT")
    ...
    val vectorAssembler = new VectorAssembler()
      .setInputCols(Array("TIME", "SL", "EEG","BP", "HR"))
      .setOutputCol("features")

    //GBT Classifier for prediction
    val gbtEstimator = new GBTClassifier()
      .setLabelCol("target")
      .setFeaturesCol("features")

    // Create the pipeline with the stages
    val pipeline = new Pipeline().setStages(Array(vectorAssembler, gbtEstimator))
